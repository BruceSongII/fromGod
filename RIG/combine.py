...
from RIG.commonly.sets import SK_createBodySets,SK_createJointSet
...
#   完成腿部合并
    legs = rig.ls('*Leg_ALL_CTRL_GRP') 
    
    if legs:
        rig.parent(legs,waistIK)
        for leg in legs:
...
#           增加肩膀follo控制           
            rig.addAttr(shoulderCon,ln = 'Arm_follow',at = 'enum',en = 'on:off:',dv = 1,k = True) 
            shoulderParent = rig.orientConstraint(chest,ArmAllCon,mo = True)[0] 
            rig.connectAttr(shoulderCon+'.Arm_follow',shoulderParent+'.'+chest+'W0')              
