...
def SK_combineJoint():
	...
	try:
		...
		#leg#
		rig.parent('Lf_leg_drv','waist1_refer')
	...
	try:
		...
		#leg#
		rig.parent('Lf_leg_drv','waist1_jnt')

		try:
			rig.parent('Rt_leg_drv','waist1_jnt')
		except:
			pass			
...
def SK_separateJoint():
	...
	try:
		...
		#leg#
		rig.parent('Lf_leg_drv','Character')
	...
	try:
		...
		try:
			rig.parent('Rt_leg_drv','Character')
		except:
			pass
		...
		# leg #
		rig.select('Lf*_leg_drv')
		allLeg=rig.ls(sl=True)
		allLeg.remove('Lf_leg_drv')
		dupLeg=allLeg
		rig.select(cl=True)
		...
		if dupLeg==[]:
			...
			for eachDup in dupLeg:
				...
				rig.parent('Lf'+ queryIndex +'_leg_drv','Character')

				rig.parent('Rt'+ queryIndex +'_leg_drv','Character')
...
def orientAndMirrorJoint ():
    ...
    try:
        rig.delete('Rt_clavicle1_jnt','Rt_leg_drv')    
    except:
        pass

    rig.mirrorJoint('Lf_clavicle1_jnt' , mirrorYZ = True ,  mirrorBehavior = True ,  searchReplace = ('Lf', 'Rt'))

    rig.mirrorJoint('Lf_leg_drv' , mirrorYZ = True ,  mirrorBehavior = True ,  searchReplace = ('Lf', 'Rt'))
...
def SK_mirrorDupJoint ():
	...
	if dupLeg==[]:
		...
		for eachDup in dupLeg:
			...
			try:
				rig.delete('Rt' + queryIndex + '_leg_drv')	
			except:
				pass

			try:
				rig.mirrorJoint('Lf' + queryIndex + '_leg_drv' , mirrorYZ = True ,  mirrorBehavior = True ,  searchReplace = ('Lf', 'Rt'))
			except:
				pass
			...
			rig.parent('Lf'+ queryIndex + '_leg_drv','waist1_jnt')

			try:
				rig.parent('Rt'+ queryIndex +'_leg_drv','waist1_jnt')
			except:
				pass	
